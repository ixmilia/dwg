    ...
    public partial class DwgDimStyleControlObject : IDictionary<string, DwgDimStyle>
    {
        ...
        internal override void OnAfterObjectRead(BitReader reader, DwgObjectCache objectCache, DwgVersionId version)
        {
            ...
            foreach (var styleHandleReference in _entityHandleReferences)
            {
                ...
                {
                    var style = objectCache.GetObject<DwgDimStyle>(reader, resolvedHandle);
        ...
        public bool TryGetValue(string key, [NotNullWhen(returnValue: true)] out DwgDimStyle? value) => ((IDictionary<string, DwgDimStyle>)_styles).TryGetValue(key, out value);
